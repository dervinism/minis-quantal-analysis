...
for iRec = 1:nRecs
  ...
  if ~isfield(fitData,'V')
    ...
    fitData.detectionParametersSim.refractoryPeriod = 2.5;
    fitData.detectionParametersSim.Amplobound = 0.02;
    fitData.detectionParametersSim.Ampupbound = 10;
    fitData.detectionParametersSim.smoothWindow = 30;
    fitData.detectionParametersSim.smoothWindowLite = 8;
    fitData.detectionParametersSim.pulseDuration = 0.5;
    fitData.detectionParametersSim.RTinterval = '10-90%';
    fitData.detectionParametersSim.downGoing = 0;
    fitData.detectionParametersSim.voltageClamp = 0;
    fitData.detectionParametersSim.sampleInterval = 0.05;
  end
    ...
    for iCombo = 1:size(combsOI,1)
      if normalise
        ...
        rtDistancesSum(iDistro,iCombo) = sum(abs( ...
          ...
          realEventRTDistrosForStats(combsOI(iCombo,2),rtInds)))./ ...
          realEventRTDistroMean(rtInds),'omitnan');
      else
        ampDistancesSum(iDistro,iCombo) = sum(abs( ...
  ...
  if parametric
    if repeatedMeasures
      ampDistancesSumSimRepeat = ampDistancesSumSim;
      ampDistancesSumSimRepeat(worstAmpDistroInd) = [];
      [~, pvalUnderfitTestAmp,~,stats] = ttest(ampDistancesSumSimRepeat', worstAmpDistroDistances', ...
        'Vartype','unequal', 'tail','right', 'alpha',alpha/nComparisons);
    else
      [~, pvalUnderfitTestAmp,~,stats] = ttest2(ampDistancesSumSim', worstAmpDistroDistances', ...
        'Vartype','unequal', 'tail','right', 'alpha',alpha/nComparisons);
    end
    fUnderfitTestAmp = stats.tstat;
    dfUnderfitTestAmp = stats.df;
    if (ampDistancesGrandSumSim/numel(ampDistancesSumSim) > mean(worstAmpDistroDistances)) && pvalUnderfitTestAmp < alpha/nComparisons
      underfitAmp = true;
    else
      underfitAmp = false;
    end
    if repeatedMeasures
      ampDistancesSumSimRepeat = ampDistancesSumSim;
      ampDistancesSumSimRepeat(bestAmpDistroInd) = [];
      [~, pvalOverfitTestAmp,~,stats] = ttest(ampDistancesSumSimRepeat', bestAmpDistroDistances', ...
        'Vartype','unequal', 'tail','left', 'alpha',alpha/nComparisons);
    else
      [~, pvalOverfitTestAmp,~,stats] = ttest2(ampDistancesSumSim', bestAmpDistroDistances', ...
        'Vartype','unequal', 'tail','left', 'alpha',alpha/nComparisons);
    end
    fOverfitTestAmp = stats.tstat;
    dfOverfitTestAmp = stats.df;
    if (ampDistancesGrandSumSim/numel(ampDistancesSumSim) < mean(bestAmpDistroDistances)) && pvalOverfitTestAmp < alpha/nComparisons
      overfitAmp = true;
    else
      overfitAmp = false;
    end
    ...
    if repeatedMeasures
      ampDistancesSumSimRepeat = ampDistancesSumSim;
      ampDistancesSumSimRepeat(worstAmpDistroInd) = [];
      [pvalUnderfitTestAmp,~,stats] = signrank(ampDistancesSumSimRepeat', worstAmpDistroDistances', ...
        'tail','right', 'method','exact', 'alpha',alpha/nComparisons);
      fUnderfitTestAmp = stats.signedrank;
    else
    ...
    if repeatedMeasures
      ampDistancesSumSimRepeat = ampDistancesSumSim;
      ampDistancesSumSimRepeat(bestAmpDistroInd) = [];
      [pvalOverfitTestAmp,~,stats] = signrank(ampDistancesSumSimRepeat', bestAmpDistroDistances', ...
        'tail','left', 'method','exact', 'alpha',alpha/nComparisons);
      fOverfitTestAmp = stats.signedrank;
    else
  ...
  if parametric
    if repeatedMeasures
      rtDistancesSumSimRepeat = rtDistancesSumSim;
      rtDistancesSumSimRepeat(worstRTDistroInd) = [];
      [~, pvalUnderfitTestRT,~,stats] = ttest(rtDistancesSumSimRepeat', worstRTDistroDistances', ...
        'Vartype','unequal', 'tail','right', 'alpha',alpha/nComparisons);
    else
      [~, pvalUnderfitTestRT,~,stats] = ttest2(rtDistancesSumSim', worstRTDistroDistances', ...
        'Vartype','unequal', 'tail','right', 'alpha',alpha/nComparisons);
    end
    fUnderfitTestRT = stats.tstat;
    dfUnderfitTestRT = stats.df;
    if (rtDistancesGrandSumSim/numel(rtDistancesSumSim) > mean(worstRTDistroDistances)) && pvalUnderfitTestRT < alpha/nComparisons
      underfitRT = true;
    else
      underfitRT = false;
    end
    if repeatedMeasures
      rtDistancesSumSimRepeat = rtDistancesSumSim;
      rtDistancesSumSimRepeat(bestRTDistroInd) = [];
      [~, pvalOverfitTestRT,~,stats] = ttest(rtDistancesSumSimRepeat', bestRTDistroDistances', ...
        'Vartype','unequal', 'tail','right', 'alpha',alpha/nComparisons);
    else
      [~, pvalOverfitTestRT,~,stats] = ttest2(rtDistancesSumSim', bestRTDistroDistances', ...
        'Vartype','unequal', 'tail','right', 'alpha',alpha/nComparisons);
    end
    fOverfitTestRT = stats.tstat;
    dfOverfitTestRT = stats.df;
    if (rtDistancesGrandSumSim/numel(rtDistancesSumSim) < mean(bestRTDistroDistances)) && pvalOverfitTestRT < alpha/nComparisons
      overfitRT = true;
    else
      overfitRT = false;
    end
    ...
    if repeatedMeasures
      rtDistancesSumSimRepeat = rtDistancesSumSim;
      rtDistancesSumSimRepeat(worstRTDistroInd) = [];
      [pvalUnderfitTestRT,~,stats] = signrank(rtDistancesSumSimRepeat', worstRTDistroDistances', ...
        'tail','right', 'method','exact', 'alpha',alpha/nComparisons);
      fUnderfitTestRT = stats.signedrank;
    else
    ...
    if repeatedMeasures
      rtDistancesSumSimRepeat = rtDistancesSumSim;
      rtDistancesSumSimRepeat(bestRTDistroInd) = [];
      [pvalOverfitTestRT,~,stats] = signrank(rtDistancesSumSimRepeat', bestRTDistroDistances', ...
        'tail','left', 'method','exact', 'alpha',alpha/nComparisons);
      fOverfitTestRT = stats.signedrank;
    else
  ...
  % Save the figure
  figName = 'DistroFittingStats';
  if normalise
    figName = [figName 'Normalised'];
  end
  if repeatedMeasures
    figName = [figName 'Repeated'];
  end
  figName = [figName 'Figures_' recIDs{iRec}];
  if ~exist(figFolder, 'dir')
    mkdir(figFolder);
  end
  figName = fullfile(figFolder, figName);
