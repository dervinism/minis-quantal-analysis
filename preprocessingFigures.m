...
for rec = 1:numel(recIDs)
  ...
  if loadAutomatic
    filterIndex = true;
    dataPathname = fullfile(dataRepo2, recIDs{rec});
    dataFilename = 'workspace.mat';
  else
    [dataFilename, dataPathname, filterIndex] = uigetfile({'*.mat','MAT files (*.mat)'}, 'Provide data file for figure retrieval', path); %#ok<*UNRCH>
  end
  if filterIndex
